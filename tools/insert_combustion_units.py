...
def insert_row(cur, row):
    columns = [
        ...
        'commissioning_date', 'unit_system_status_id', 'plant_block_name',
        'unit_operational_status_id', 'not_present_in_migrated_units',
        'weic_not_available', 'plant_number_not_available', 'energy_source_id',
        'gross_capacity', 'net_nominal_capacity', 'remote_control_nb',
        'supply_type_id', 'plant_name', 'primary_fuel_id',
        'emergency_power_generator', 'kwk_registration_number',
        'technology_id', 'wkb_geometry'
    ]

    sql = f'''
        INSERT INTO de_combustion_units ({", ".join(columns)})
...
def read_combustion_units(conn, src):
    cur = conn.cursor()

    context = etree.iterparse(src, events=('end',), tag='EinheitVerbrennung')
    ...
    for event, elem in context:
        unit = {
            ...
            'remote_control_nb': parse_value(elem, 'FernsteuerbarkeitNb'),
            ...
            'emergency_power_generator': parse_value(elem, 'Notstromaggregat'),
...
def main(env, src, verbose, debug):
    ...
    read_combustion_units(conn, Path(src))
